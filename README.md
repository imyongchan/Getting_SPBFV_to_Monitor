...
- (a) **공간말초혈류속도(SPBFV)**: 손목의 요골동맥에 위치한 PPG-1과 손가락 모세혈관에 위치한 PPG-2로부터 동시에 측정된 두 맥파의 피크시간 차이(△τ)를 손길이(LH)로 나누어 정의합니다.
- (b)(c) PPG-1 첫 번째 피크와 PPG-2 두 번째 피크의 시간 차이(△τ1i)를 정의하며, i는 5번까지 연속 카운트를 나타냄 △τ의 평균값(△τave)을 계산하여 손길이(LH)로 나눔으로써 SPBFV 계산합니다.
...
- 스마트 혈류속도 측정기로 손목과 손가락 데이터의 피크지점을 검출하여 심박수 측정하고 공간말초혈류속도(SPBFV)는 계산 알고리즘을 통해 산출합니다.
- 실시간 심박수와 공간말초혈류속도는 Wifi를 통해 MQTT 통신으로 라즈베리파이5 서버에 전송되고 스마트폰/PC에서 기기측정 및 결과 확인 가능합니다.
...
- 본 플랫폼은 **새로운 혈역학 생체정보인 공간말초혈류속도(SPBFV)를 제공함으로써**, 실시간 측정으로 비대면 진료 환경에서도 유용하게 활용될 수 있으며, 디지털 헬스케어 시대에 맞춘 혁신적인 **심뇌혈관 관리 솔루션으로 자리매김할 수 있을 것으로 보입니다.**

---

## ‼ 배운점 
- 배운점 : 약 9개월간 쉽게 진전이 없어 포기하고 싶은 순간도 있었지만, 끈기 있게 원인을 추적한 끝에 해결 아이디어를 찾아낼 수 있었습니다. 이 경험을 통해 기술을 누가, 왜 필요로 하는지를 먼저 이해해야 어떻게 만들지가 보인다는 점을 배웠고, 매주 기업 담당자 및 연구실과 미팅하며 결과를 정리·발표하고 피드백을 반영하는 과정에서 외부 협업 커뮤니케이션 역량을 기를 수 있었습니다.
---
